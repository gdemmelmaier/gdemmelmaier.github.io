...
#Calculates how many of the opponents stones that are blocked and subtracts
#the number of stones of the player that is blocked.
...
#Heuristic function to give a score value to the input board
def heuristic(board,player,turn,difficulty):
    ...
    if(difficulty==3):
        if turn<18:
            ...
            reward=[43,10,0,20] #rewardfunction for phase 2
    elif(difficulty==2):
        if turn<18:
            reward=[25,0,0,22]
        elif turn%3==0:
   ...
   # and if there's a 3 piece configuration12
    ...
    #calculates the number of two in a row with a empty spot in the last place
    for i in range(24):
        if(board[i]==-1):
            board[i]=1
            if(isRow(i,board)):
...
#  Make-a-move function for phase one
def move_1(board, player):
    ...
    for i in range(24): #loops thorgh all the positions on the board
        if(board[i]<0): #checks if position is empty
...
# If player has three in a row, this function removes a stone.
...
#  Make-a-move function for phase two
...
#  Make-a-move function for phase three
...
def adjacentLocations_9mensmorris(position): #adjecent locations without the diagonals
    adjacent = [
        [1, 7],
        [0, 2, 9],
        [1, 3],
...
def isPlayer(player, board, p1, p2):
    if (board[p1] == player and board[p2] == player):
        ...
        return False
#Checks if player is at postion
...
def checkNextRow(position, board, player):
    row = [
        (isPlayer(player, board, 1, 2) or isPlayer(player, board, 8, 16) or isPlayer(player, board, 6, 7)),
        (isPlayer(player, board, 0, 2) or isPlayer(player, board, 9, 17)),
        (isPlayer(player, board, 0, 1) or isPlayer(player, board, 3, 4) or isPlayer(player, board, 10, 18)),
        (isPlayer(player, board, 2, 4) or isPlayer(player, board, 11, 19)),
        (isPlayer(player, board, 2, 3) or isPlayer(player, board, 12, 20) or isPlayer(player, board, 5, 6)),
        (isPlayer(player, board, 4, 6) or isPlayer(player, board, 13, 21)),
        (isPlayer(player, board, 4, 5) or isPlayer(player, board, 14, 22) or isPlayer(player, board, 0, 7)),
        (isPlayer(player, board, 0, 6) or isPlayer(player, board, 15, 23)),
        (isPlayer(player, board, 9, 10) or isPlayer(player, board, 0, 16) or isPlayer(player, board, 14, 15)),
        (isPlayer(player, board, 8, 10) or isPlayer(player, board, 1, 17)),
        (isPlayer(player, board, 8, 9) or isPlayer(player, board, 11, 12) or isPlayer(player, board, 2, 18)),
        (isPlayer(player, board, 3, 19) or isPlayer(player, board, 10, 12)),
        (isPlayer(player, board, 20, 4) or isPlayer(player, board, 10, 11) or isPlayer(player, board, 13, 14)),
        (isPlayer(player, board, 5, 21) or isPlayer(player, board, 12, 14)),
        (isPlayer(player, board, 13, 12) or isPlayer(player, board, 15, 8) or isPlayer(player, board, 6, 22)),
        (isPlayer(player, board, 8, 14) or isPlayer(player, board, 7, 23)),
        (isPlayer(player, board, 17, 18) or isPlayer(player, board, 22, 23) or isPlayer(player, board, 0, 8)),
        (isPlayer(player, board, 1, 9) or isPlayer(player, board, 16, 18)),
        (isPlayer(player, board, 16, 17) or isPlayer(player, board, 19, 20) or isPlayer(player, board, 2, 10)),
        (isPlayer(player, board, 18, 20) or isPlayer(player, board, 3, 11)),
        (isPlayer(player, board, 18, 19) or isPlayer(player, board, 12, 4) or isPlayer(player, board, 22, 21)),
        (isPlayer(player, board, 20, 22) or isPlayer(player, board, 5, 13)),
        (isPlayer(player, board, 20, 21) or isPlayer(player, board, 16, 23) or isPlayer(player, board, 6, 14)),
        (isPlayer(player, board, 22, 16) or isPlayer(player, board, 7, 15))
    ]

    return row[position]

def checkNextRow_9mensmorris(position, board, player):
    row = [
...
# Used by hueristic function to determine worth
...
#alpha and beta are parameters for pruning

def minmaxstart(board,depth,turn,player,alpha,beta,difficulty,timeStart):

    diffTime = 285 # Maximum time for AI to make a move [seconds]
    endTime = time.time() # End time
    if (endTime - timeStart >= diffTime): # Check max-time limit
        return 0
    if depth==0 :
    ...
    if player==1:#this is maximizing player
        ...
        for newVec in moves:
            value=max(value,minmaxstart(newVec,depth-1,turn+1,0,alpha,beta,difficulty,timeStart))
        ...
        for newVec in moves:
            value=min(value,minmaxstart(newVec,depth-1,turn+1,1,alpha,beta,difficulty,timeStart))
            beta=min(beta,value)
            if alpha>=beta:
                break
        return value
...
# Find the next, best move evaluated by the hueristic function.
def findNextMove(board,player,turn,depth,difficulty):

    timeStart = time.time() # Initiate time-counting, to esnure time limit is held.
    bestMove=[]
    ...
    for newVec in moves:
        ...
        randDepth = 0
        if (difficulty == 2 and rand >= 50):
            print('another depth level added---------!!!!!!!!!!!1')
            randDepth = 1
        currentMove=minmaxstart(newVec,depth + randDepth,turn+1,0,-10000000,1000000000,difficulty,timeStart)
...
def runUUGame(filename):
    ...
    except IOError: #If there is no file, start a new game
    ...
    if player == 0:
        board = invert_board(findNextMove(invert_board(board), player, turn, difficulty + 2,difficulty))
    else:
        board = findNextMove(board, player, turn, difficulty + 2, difficulty)
    #print(board)
    #print(invert_board(board))
    writeOutputFile(filename, board, 1 - player, turn + 1, difficulty)


#running the code below this comment will run the code for different difficulties of the ai
#playing against each other and then printing who is winning.
'''
board=[]
for p in range(24):
    board.append(-1)
...
for p in range(24):
    board.append(-1)
print("diff 3 starts against diff 1")
turn=0
while(True):
    board=findNextMove(board,1,turn, 3,3)
    turn+=1
...
for p in range(24):
    board.append(-1)
print("diff 1 starts against diff 3")
turn=0
while(True):
    board=findNextMove(board,1,turn, 1,1)
    turn+=1
