...
class GamePlatform:
    ...
    def play(self, match):
        ...
        if match.get_black_player().is_ai():
            return playAIGame.playGameAI(False, "W ", match.get_black_player_name(), match.get_white_player_name(), match.get_black_player().difficulty)
        elif match.get_white_player().is_ai():
            return playAIGame.playGameAI(False, "B ", match.get_black_player_name(), match.get_white_player_name(), match.get_white_player().difficulty)
        else:
            return game_platform.play_game(True, match.get_black_player_name(), match.get_white_player_name(), "B")

    def setup(self, black, white):
        self.wn = white
        self.bn = black
        game_platform.initialize()

class ProjectSE:
    def __init__(self,cb=ConfigurationBuilder(),pltfrm=GamePlatform(),gm=GameManager()):
        self.cb = cb
    ...
    def intro_menu_choice(self):
        ...
        print('Welcome to the UU-game, the Tournament will now start.')
        return "Tournament" # Always return Tournament, the game-platform handles mode decision
    ...
    def play_match(self, match):
        ...
        if board_state.is_draw():
            ...
            return winner
        """
        winner = self.platform.play(match)
        if winner == "Black":
